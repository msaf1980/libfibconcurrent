...
static inline fifo_steal_buffer_t* fifo_steal_buffer_create(uint32_t power_of_2_size)
{
    ...
    const uint32_t required_size = (uint32_t) (sizeof(fifo_steal_buffer_t) + size * sizeof(void*));
    fifo_steal_buffer_t* ret;
    assert(power_of_2_size && power_of_2_size < 32);
    ret = (fifo_steal_buffer_t*)malloc(required_size);
    if(ret) {
...
static inline int fifo_steal_buffer_pop(fifo_steal_buffer_t* fifo, void** out)
{
    ...
    if(high > fifo->low) {
        ...
        index = (new_low - 1) & fifo->power_of_2_mod;
...
static inline sharded_fifo_steal_buffer_t* sharded_fifo_steal_buffer_create(uint32_t num_shards, uint32_t shard_power_of_2_size)
{
    ...
    const size_t memory_per_shard = sizeof(fifo_steal_buffer_t) + shard_size * sizeof(void*);
    const size_t total_memory = sizeof(sharded_fifo_steal_buffer_t) + num_shards * memory_per_shard;
