...
//printf("action: %d\n", action);
        if(action < 4) {
            ...
            n->data = (void*)i;
            assert(sharded_fifo_steal_buffer_push(my_fifo, n));
            ++my_data->push_count;
        } else {
            ...
            while(tries > 0) {
                ...
                int ret;
                do {
                    ret = sharded_fifo_steal_buffer_steal(steal_fifo, (uint32_t) thread_id, (void**)&n);
...
int main(int argc, char* argv[])
{
    ...
    fifo = calloc((size_t) NUM_THREADS, sizeof(*fifo));
    data = calloc((size_t) NUM_THREADS, sizeof(*data));
    pthread_barrier_init(&barrier, NULL, (unsigned int) NUM_THREADS);

    for(i = 0; i < NUM_THREADS; ++i) {
        fifo[i] = sharded_fifo_steal_buffer_create((uint32_t) NUM_THREADS, 20);
    }
    ...
    printf("timing: %d threads %d events %d work %lf seconds (%.2f ns per item)\n",
        NUM_THREADS, PER_THREAD_COUNT, WORK_FACTOR, us / 1000000.0,
        us * 1000.0 / (double) (total.push_count + total.pop_count + total.empty_count + total.steal_count + total.steal_count + total.attempt_count)
    );
