...
#include <fibconcurrent/dist_fifo.h>
...
size_t PER_THREAD_COUNT = 100000;
int WORK_FACTOR = 0;
...
void* run_function(void* param)
{
    ...
    for(i = 0; i < PER_THREAD_COUNT; ++i) {
        ...
        if(action < 4) {
            ...
            int stole = 0;
            while(tries > 0) {
                dist_fifo_t* const steal_fifo = &(fifo[(size_t) j % NUM_THREADS]);
                dist_fifo_node_t* n;
                do {
...
int main(int argc, char* argv[])
{
    ...
    printf("timing: %lu threads %lu events %d work %lf seconds (%.2f ns per item)\n",
        NUM_THREADS, PER_THREAD_COUNT, WORK_FACTOR, us / 1000000.0, us * 1000.0 / (double) (total.push_count + total.push_count + total.steal_count));
