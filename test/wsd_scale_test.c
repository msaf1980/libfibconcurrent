...
void* run_function(void* param)
{
    ...
    for(i = 0; i < PER_THREAD_COUNT; ++i) {
        ...
        if(action < 4) {
            ...
            int stole = 0;
            while(tries > 0) {
                wsd_work_stealing_deque_t* const steal_fifo = fifo[(size_t) j % NUM_THREADS];
                node_t* n;
                do {
...
int main(int argc, char* argv[])
{
    ...
    for(i = 0; i < NUM_THREADS; ++i) {
        fifo[i] = wsd_work_stealing_deque_create(0);
    }
    ...
    printf("timing: %lu threads %lu events %d work %lf seconds (%.2f ns per item)\n",
        NUM_THREADS, PER_THREAD_COUNT, WORK_FACTOR, us / 1000000, us * 1000.0 / (double) (total.push_count + total.push_count + total.steal_count));
